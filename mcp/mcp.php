...
function sbmcp_register_mcp_route() {
    ...
    // DEPRECATED: pressbridge/v1 namespace kept for back-compat with existing connectors. Tracked for removal in issue #9.
    ...
    if (!get_option('sbmcp_api_disabled')) {
        ...
        if ($token) {
            ...
            // DEPRECATED back-compat. Tracked for removal in issue #9.
...
function sbmcp_mcp_all_tools(): array {
    return [
        ...
        'upload_media' => array_merge(['name' => 'upload_media', 'description' => 'Upload media from a URL or base64 string. Provide url, or base64 + filename.', 'inputSchema' => ['type' => 'object', 'properties' => ['url' => ['type' => 'string', 'description' => 'Source URL to fetch. http/https only; private and loopback addresses are blocked.'], 'base64' => ['type' => 'string'], 'filename' => ['type' => 'string', 'description' => 'Name to store the file under. Required with base64. Optional with url, where it overrides the name derived from the URL and must carry an image extension; omit it to keep the name the URL implies.'], 'title' => ['type' => 'string', 'description' => 'Attachment title. Defaults to the filename.']]]], _sbmcp_ann_write()),
        ...
        'update_option' => array_merge(['name' => 'update_option', 'description' => 'Write a WordPress option. This is the recommended way to write options: it goes through update_option(), so the object cache and any registered filters stay consistent with what is stored. Writing to the options table with direct SQL bypasses the object cache, leaving a persistent cache (Redis, Memcached) serving the old value until it is flushed.', 'inputSchema' => ['type' => 'object', 'properties' => ['key' => ['type' => 'string'], 'value' => ['type' => 'string', 'description' => 'Option value. Stored exactly as given: strings, numbers and booleans are written as-is. JSON is NOT decoded automatically. To store an object or array, pass its JSON encoding here and set json: true.'], 'json' => ['type' => 'boolean', 'description' => 'How to treat a value that looks like JSON. true: decode it and store the resulting array, so WordPress serializes it the way PHP code reading the option expects - use this for any option whose stored form is a PHP array. false: store the value as a literal string, unchanged. Omit it entirely and a JSON-looking value is rejected rather than guessed at, so absent and false are not the same thing.']], 'required' => ['key', 'value']]], _sbmcp_ann_write()),
...
function sbmcp_mcp_tools_call($id, $params) {
    ...
    switch ($name) {
        ...
        case 'update_option':
            // json is forwarded only when present: absent and false are distinct
            // (reject a JSON-looking value vs. store it literally), so defaulting
            // it here would turn every unflagged call into an explicit "literal".
            $opt_body = ['key' => $input['key'] ?? '', 'value' => $input['value'] ?? null];
            if (array_key_exists('json', $input)) $opt_body['json'] = $input['json'];
            return sbmcp_mcp_cb($id, sbmcp_update_option($json_req($opt_body)));
        case 'list_options':
        ...
        case 'create_menu_item': $mid = (int) ($input['menu_id'] ?? 0); unset($input['menu_id']); $req = $json_req($input); $req['menu_id'] = $mid; return sbmcp_mcp_cb($id, sbmcp_create_menu_item($req));
