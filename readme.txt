...
= 3.1.0 =
* Security: an add-on's REST routes could bypass read-only mode, the OAuth scope and capability gate, and the activity log entirely. An add-on registered its routes directly with WordPress, and nothing required those routes to pass through this plugin's guard, so a connection granted only read access could reach an add-on's write tools and no record of the call was kept. Guarding is now structural rather than a convention an add-on has to follow: any route in the `strifebridge/v1` or `pressbridge/v1` namespaces whose callback did not come from this plugin's own guard is refused with a 403 that names the fix, and the refusal is recorded as denied. A tool route can no longer be registered unguarded. **This refuses StrifeBridge MCP Pro 2.4.0's REST routes — Pro must be updated to 2.4.1 at the same time.** The MCP path, which is what every AI assistant actually connects through, is unaffected in either direction.
* Security: tool group toggles did not gate an add-on's tools on the MCP path. A tool belonging to a group you had switched off in Settings could still be called by name and would run. Group toggles now apply to every registered tool, whoever registered it.
* Security: an add-on tool this plugin could not classify fell through to a permissive scope. Unregistered tools now require `mcp:admin`, so an unrecognised tool fails closed rather than being reachable by a connection approved only for content.
* New: a single tool registry. A tool's group, whether it reads or writes, the capability it requires, its OAuth scope, and the arguments that may be logged are all declared once, in one place. Previously the same tool had to be described in four separate lists, and an add-on that extended some but not all of them silently lost the guards it missed — which is the common cause behind the four issues above. Add-ons declare each tool once through `sbmcp_tool_registry` and get every guard. Undeclared tools are treated as writes requiring `manage_options`.
* Change (behaviour): Safe Mode "Trash instead of delete" now means what it says for everything, not only posts. Deleting media goes to the trash where WordPress has media trash enabled, and is refused where it does not. Tools with nothing to trash to — terms, menu items, plugins, and any add-on tool that declares itself irreversible — are refused outright, with the message naming the setting responsible. **On a site with this setting switched on, `delete_term`, `delete_menu_item` and `delete_plugin` will now be refused where they previously deleted permanently.** Switch the setting off if you want those calls to go through.
* Security: refresh token rotation is now atomic. A refresh token is claimed in a single conditional write, so two requests presenting the same token concurrently can no longer both succeed. Presenting a refresh token that has already been rotated is treated as a compromise and revokes every access and refresh token for that application and account.
* New: the activity log records which WordPress account and which connected application performed each action, and Settings shows a Who column. Entries were previously grouped by a token hint, which rotates hourly under OAuth and so grouped nothing.
* Fix: an activity log write that fails now surfaces as an admin notice instead of being swallowed. An empty Recent Activity panel previously meant either that nothing had happened or that logging was broken, with no way to tell the two apart.
* Fix: `server_ping`, `get_site_info` and `list_plugins` are readable with a read-only grant. All three only report, but were classified as writes, so a connection approved for reading could not call them and read-only mode refused them.
* Housekeeping: the Pro promotions in Settings and on the Plugins screen are hidden once Pro is installed, and the activity panel states the retention actually in effect instead of advertising Pro to someone who already has it.
...
= 3.1.0 =
Security release, recommended for all users, and **required reading if you run StrifeBridge MCP Pro: 3.1.0 and Pro 2.4.1 must be installed together.** This release refuses any REST route that does not pass through the plugin's guard, and Pro 2.4.0's routes do not — so free 3.1.0 alongside Pro 2.4.0 will refuse Pro's REST tools until Pro is updated. The MCP path that AI assistants connect through is unaffected either way. Closes a class of bypass where an add-on's REST routes skipped read-only mode, OAuth scope and capability checks, and the activity log; where tool group toggles did not hold for add-on tools on the MCP path; and where an unclassified tool fell through to a permissive scope. Also makes refresh token rotation atomic with reuse detection. One behaviour change to note: Safe Mode "Trash instead of delete" now covers terms, menu items and media rather than posts alone, so on a site with that setting on, `delete_term`, `delete_menu_item` and `delete_plugin` will now be refused instead of deleting permanently.
